...
class ZMQ_SubscriberThread(threading.Thread):
    ...
    def __init__(self, host: str, port: int, context: Optional[zmq.Context] = None, decoder: Optional[Callable[[bytes], Any]] = None):
        ...
        self._started = threading.Event()
        self._decoder = decoder or self._decode_image
        # for fps calculation
        self._image_fps = 0.0
        self._t0 = None
        self._fps_count = 0
            ...
            while self._running:
                ...
                if self._socket in events:
                    try:
                        ...
                        img_numpy = self._decoder(img_bytes)
...
class ZMQ_SubscriberManager:
    ...
    _subscriber_threads: Dict[Tuple[str, int, str], ZMQ_SubscriberThread] = {}
    _lock = threading.Lock()
    _running = True
    ...
    def _create_subscriber_thread(self, host: str, port: int, decoder: Optional[Callable[[bytes], Any]] = None) -> ZMQ_SubscriberThread:
        try:
            subscriber_thread = ZMQ_SubscriberThread(host, port, self._context, decoder=decoder)
    ...
    def _get_subscriber_thread(self, host: str, port: int, decoder_kind: str, decoder: Optional[Callable[[bytes], Any]] = None) -> ZMQ_SubscriberThread:
        key = (host, port, decoder_kind)
        with self._lock:
            if key not in self._subscriber_threads:
                self._subscriber_threads[key] = self._create_subscriber_thread(host, port, decoder=decoder)
    ...
    def subscribe(self, host: str, port: int) -> Tuple[Optional[np.ndarray], float]:
        ...
        subscriber_thread = self._get_subscriber_thread(host, port, decoder_kind="jpeg", decoder=None)
        return subscriber_thread.recv(), subscriber_thread.get_fps()

    def subscribe_depth(self, host: str, port: int, image_shape: Tuple[int, int]) -> Tuple[Optional[np.ndarray], float]:
        """Subscribe to a depth stream published as raw uint16 bytes.
        ...
        Returns:
            depth: uint16 ndarray of shape (H, W), or None
            fps: receive FPS
        """
        height, width = int(image_shape[0]), int(image_shape[1])

        def _decode_depth(depth_bytes: bytes) -> Optional[np.ndarray]:
            if depth_bytes is None:
                return None
            arr = np.frombuffer(depth_bytes, dtype=np.uint16)
            if arr.size != height * width:
                logger_mp.warning(
                    f"[ZMQ_SubscriberThread] Depth payload size mismatch: got {arr.size}, expected {height * width}"
                )
                return None
            return arr.reshape((height, width))

        decoder_kind = f"depth:{height}x{width}"
        subscriber_thread = self._get_subscriber_thread(host, port, decoder_kind=decoder_kind, decoder=_decode_depth)
        return subscriber_thread.recv(), subscriber_thread.get_fps()
...
class ImageClient:
    def __init__(self, host="192.168.123.164", request_port=60000):
        ...
        # Pre-subscribe to enabled depth streams if present
        for cam_topic, cfg in self._cam_config.items():
            try:
                if cfg.get('enable_depth_zmq', False):
                    depth_port = cfg.get('depth_zmq_port', None)
                    shape = cfg.get('depth_image_shape', None) or cfg.get('image_shape', None)
                    if depth_port is None or shape is None:
                        continue
                    self._subscriber_manager.subscribe_depth(self._host, int(depth_port), tuple(shape))
            except Exception:
                # Keep client robust to partial configs
                continue
    ...
    def get_depth_frame(self, cam_topic: str) -> Tuple[Optional[np.ndarray], float]:
        """Get the latest depth frame for a camera topic.

        Requires server-side config:
          enable_depth_zmq: true
          depth_zmq_port: <port>
        Depth is returned as uint16 ndarray with shape depth_image_shape (or image_shape).
        """
        cfg = self._cam_config.get(cam_topic)
        if not cfg:
            return None, 0.0
        if not cfg.get('enable_depth_zmq', False):
            return None, 0.0
        depth_port = cfg.get('depth_zmq_port', None)
        shape = cfg.get('depth_image_shape', None) or cfg.get('image_shape', None)
        if depth_port is None or shape is None:
            return None, 0.0
        return self._subscriber_manager.subscribe_depth(self._host, int(depth_port), tuple(shape))
    ...
    while running:
        if cam_config['head_camera']['enable_zmq']:
            ...
            if head_img is not None:
                ...
                logger_mp.debug(
                    f"Head Camera Binocular: {cam_config.get('head_camera', {}).get('binocular', None)}"
                )
                cv2.imshow("Head Camera", head_img)

        # Optional: show depth if configured
        if cam_config.get('head_camera', {}).get('enable_depth_zmq', False):
            depth_img, depth_fps = client.get_depth_frame('head_camera')
            if depth_img is not None:
                logger_mp.info(f"Head Depth FPS: {depth_fps:.2f}")
